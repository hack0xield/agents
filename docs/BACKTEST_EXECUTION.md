...
Margin-zone studies — pivots, envelopes, crossings, no entries or P&L — are
still read from the run directories and reported as `kind: "study"` with
`win_rate` null. Nothing runs a new one: `backtests.run` is the only runner.

Keeping them apart from strategy backtests still matters. A reach rate and a
win rate are both percentages in the nineties, and "97% reached the first zone"
reads like a 97% win rate to anyone who asked about profitability.

## Reverting

1. Delete `backtests.run` and `backtests.list_strategies` from
